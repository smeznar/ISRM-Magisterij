(* Original source: https://github.com/MatevzFa/sml-unittest/blob/master/unittest.sml *)
...
fun assert_eq_real (a: real, b: real) =
    assert (Real.abs (a - b) < 0.001);

fun assert_raises (f, e: exn) =
    let 
        val x = f ()
    in
        FAIL
    end
    handle thrown_e => assert_eq (exnName thrown_e, exnName e);
